...
def generate_text_task(
    ...
    publish_progress(
        ...
        status="running",
        progress=5,
        stage_text="Preparing text task",
    ...
    try:
        ...
        publish_progress(
            ...
            status="running",
            progress=15,
            stage_text="Calling text provider",
            retry_count=self.request.retries,
            celery_task_id=self.request.id,
        )
        call = resolve_callable(
            "app.services.doubao",
            ("generate_text", "call_doubao", "submit_text_generation", "generate"),
        )
        result = invoke_callable(call, payload, api_key=api_key, task_id=task_id, user_id=user_id)
        maybe_charge(transaction_id)
        result = persist_result_to_oss(result, "text")
        publish_progress(
            task_id,
            status="running",
            progress=100,
            stage_text="Text generation complete",
            retry_count=self.request.retries,
            celery_task_id=self.request.id,
        )
        ...
        if retryable and self.request.retries < MAX_RETRIES:
            publish_progress(
                ...
                status="retrying",
                progress=15,
                stage_text=f"Retrying text task ({self.request.retries + 1}/{MAX_RETRIES})",
