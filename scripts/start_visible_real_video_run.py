...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start a visible real-provider video production run.")
    ...
    parser.add_argument(
        "--goal",
        default=(
            "Create a 15-second cinematic short-drama demo: on a rainy night, the lead receives "
            "a decisive message under a streetlight, absorbs it silently, then leaves."
        ),
    )
    parser.add_argument("--execute", action="store_true", help="Actually create the project and start real provider tasks.")
        ...
        async with session.begin():
            ...
            await session.execute(
                ...
                ),
                {
                    ...
                    "user_id": user_id,
                    "name": "Agent visible real video demo",
                },
            )
        ...
        json.dumps(
            {
                "project_id": project_id,
                "user_id": user_id,
                "project_name": "Agent visible real video demo",
